  ...
  if (!fs.existsSync(app.pm_exec_path)) {
    ...
    if ((ckd = which(app.script))) {
      if (typeof(ckd) !== 'string')
        ckd = ckd.toString();
      if(cst.IS_WINDOWS) {
        // If the script is npm or npm.cmd on windows
        if(/^npm(\.cmd)?(\s|$)/i.test(app.script)) {
          ckd = ckd.replace(/npm.cmd$/i, 'node_modules\\npm\\bin\\npm-cli.js');
        }
        // If the script is npx or npx.cmd on windows
        else if(/^npx(\.cmd)?(\s|$)/i.test(app.script)) {
          ckd = ckd.replace(/npx.cmd$/i, 'node_modules\\npm\\bin\\npx-cli.js');
        }
      }
